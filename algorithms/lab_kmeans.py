...
class LABKMeansPlusPlus:
    ...
    def kmeans_plus_plus_initialization(self, pixels_lab):
        """K-Means++ initialization in LAB space"""
        rng = np.random.RandomState(self.random_state)
        idx_mask = np.zeros(shape=pixels_lab.shape[0]).astype('bool')
        data_idxs = np.arange(pixels_lab.shape[0])
        ...
        for _ in range(1, self.k):
            min_centroid_dist = (np.linalg.norm(
                pixels_lab[~idx_mask, np.newaxis, :] - pixels_lab[np.newaxis, idx_mask, :],
                axis=-1
            )**2).min(axis=-1)
        ...
        return pixels_lab[idx_mask].astype(float)
    ...
    def fit(self, pixels_rgb):
        ...
        self.centroids_lab = self.kmeans_plus_plus_initialization(pixels_lab)
...
if __name__ == "__main__":
    ...
    # Run LAB K-Means++
    kmeans = LABKMeansPlusPlus(k=10, max_iter=100, random_state=42)
    kmeans.fit(pixels)
